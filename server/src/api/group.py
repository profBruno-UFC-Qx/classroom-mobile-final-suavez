...
async def create_group(group: Group, db: Session = Depends(get_db)):
    db_group = db.query(DBGroup).filter(DBGroup.id == group.id).first()
    if db_group:
...
@router.get("/{group_id}/activities", response_model=List[StudyActivity])
async def get_group_activities(group_id: str, db: Session = Depends(get_db)):
    all_activities = db.query(DBStudyActivity).all()
    filtered = [act for act in all_activities if group_id in act.group_ids]
    
    result = []
    for act in sorted(filtered, key=lambda x: x.created_at_millis, reverse=True):
        result.append(StudyActivity(
            id=act.id,
...
async def join_group(
    ...
    group = db.query(DBGroup).filter(DBGroup.invite_code == payload.invite_code).first()
