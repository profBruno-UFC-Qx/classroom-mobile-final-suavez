...
from src.security import create_access_token, get_current_user, get_password_hash, verify_password
...
async def register_user(user_in: UserRegister, db: Session = Depends(get_db)):
    user_exists = db.query(DBUser).filter(
        (DBUser.email == user_in.email) | (DBUser.username == user_in.username)
    ).first()
    ...
    initials = "".join([n[0] for n in user_in.name.split()[:2]]).upper()
