...
@router.get("/{user_id}/activities", response_model=List[StudyActivity])
async def get_user_activities(user_id: str, db: Session = Depends(get_db)):
    activities = db.query(DBStudyActivity).filter(
        DBStudyActivity.author_id == user_id
    ).order_by(DBStudyActivity.created_at_millis.desc()).all()
